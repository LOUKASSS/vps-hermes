...
One Hermes agent (gateway + dashboard) behind Traefik (HTTPS via Cloudflare DNS-01),
a thin agent image (`gh` / `tmux` / `jq` / `psql`, **no** coding CLIs), PostgreSQL for
your own structured data, an optional Obsidian Sync sidecar on the public `node` image,
host-persistent storage the agent and Orca share, nightly encrypted backups to Backblaze B2,
weekly `docker compose pull` + thin rebuild, a self-healing timer, and an optional
[Orca](https://www.onorca.dev) remote server **on the host** as user **`hermes`**.
...
Tailnet ──6768──▶ orca.service on the host (User=hermes, cwd /srv/hermes/projects)

/srv/hermes/                 (owned by the operator user `hermes`, uid 1000)
├── stack/                   this repo: compose, scripts, .env  (not mounted in the agent)
├── data/       → /opt/data  agent HOME: config, sessions, credentials, skills, private/, mcp/
├── projects/   → /workspace  repos + vault + db + scratch; shared with Orca
├── orca/                    Orca HOME (0700, not mounted)
├── traefik/    → acme.json
├── postgres/   data/ (live cluster) + dumps/ (daily pg_dumpall)
└── obsidian/   → sidecar HOME (Obsidian Sync credentials)
```

Three HOMEs, one uid: `/home/hermes` (SSH), `/srv/hermes/orca` (Orca), `/srv/hermes/data/home`
(agent tools). Coding CLIs (`claude` / `codex` / `grok`) run on the host via Orca, never in the
container. DNS and Obsidian use public images (`4km3/dnsmasq:2.90-r3`, `node:22-bookworm-slim`).
...
`DESKTOP_PASSWORD`, `DESKTOP_SECRET`), creates `/srv/hermes/projects`, `data/private`,
...
`HERMES_UID/GID` from the `hermes` user. Fresh `.env` uses `HERMES_WORKSPACE_DIR=/srv/hermes/projects`
...
Coding CLIs live on the **host**: `sudo ./orca.sh login claude|codex|grok` (not in the agent
container). `auth.sh claude|codex|grok` prints that pointer.

Menu numbers work as arguments too (`sudo ./auth.sh 5`). `obsidian` and `status` (as arguments)
...
`openai-codex`, `xai-oauth`). Interactive Claude / Codex / Grok sessions are Orca on the host.
...
and the agent binds) with its own `HOME` (`ORCA_HOME`, default `/srv/hermes/orca`, mode 0700 —
**not** `/home/hermes`, **not** mounted in the agent). Sessions cwd is
`/srv/hermes/projects` (the same tree the agent sees as `/workspace`).
...
git hooks in `projects/` are overridden for Orca by `GIT_CONFIG_COUNT` in `/etc/orca.env`
(git `-c` rank). Treat `projects/` as **untrusted for sudo** (Makefiles / `deploy.sh` remain a
...
**Editing this stack from Orca.** Open `/srv/hermes/stack` as a project: sessions can change it
...
**Sharing code with the agent.** Same tree: `/srv/hermes/projects/projects/<repo>` is
`/workspace/projects/<repo>` in the container. Do not flatten that extra `projects/` segment.
...
The agent writes Markdown into `/workspace/<OBSIDIAN_VAULT_DIR>` (default `vault`, host
`/srv/hermes/projects/vault`). The `obsidian-sync` sidecar is the official
...
`projects/` (or the path in `HERMES_WORKSPACE_DIR`), `obsidian/`, `postgres/dumps/`,
`traefik/acme.json` and `stack/.env` — minus `node_modules`, venvs, caches, browser profiles.
...
Pre-cut-over snapshots restore the files tree under `…/srv/hermes/workspace/` (restic keeps host
paths; there is no `workspace` → `projects` symlink). If the restore target has no `projects/`,
rsync that tree onto `$HERMES_WORKSPACE_DIR` (printed by `backup.sh restore`).
...
- Put files in `/srv/hermes/projects` on the VPS → visible as `/workspace` (the agent's cwd).
  Repos stay at `/workspace/projects/<repo>` (do not flatten).
...
sudo ./update.sh                            # pull public images + thin agent rebuild, recreate (auto-rollback if unhealthy)
...
`compose build --pull hermes-agent` (cache allowed — no npm CLIs in the image) and
`compose pull --ignore-buildable`. Typical duration is ~30 s–2 min instead of a no-cache rebuild
of three coding CLIs. If `hermes-agent` is not healthy within a few minutes it rolls back to
...
`hermes/Dockerfile` `FROM`. When Orca is installed, `update.sh` ends with `orca.sh update`.
...
runs, and while `/srv/hermes/stack/.maintenance` exists — **touch that file before
...
| Coding CLI logins | `sudo ./orca.sh login claude\|codex\|grok` (and `sudo ./orca.sh creds` after `auth.sh gh`) |
...
| Path | Purpose |
|---|---|
| `docker-compose.yml` | docker-socket-proxy, traefik, hermes-agent (thin build; gateway + dashboard), postgres, dns (`4km3/dnsmasq`), obsidian-sync (`node:22-bookworm-slim`, profile `obsidian`) |
| `dns/dnsmasq.conf` | bind-mounted into the public dnsmasq image (zone via compose `command`) |
| `hermes/Dockerfile` | `FROM nousresearch/hermes-agent:latest` + `gh`, `tmux`, `jq`, `psql`, `less`, `nano` — no coding CLIs |
| `obsidian/entrypoint.sh` | install `obsidian-headless@0.0.14` into the volume, then `ob` |
| `orca.sh` / `orca/orca.service` | Orca on the host as `User=hermes`, cwd `projects/`, `GIT_CONFIG_COUNT` |
...
| `lib/common.sh` | shared helpers (workspace fallback stays `/srv/hermes/workspace` until `.env` is patched) |
