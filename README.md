...
| Piece | Where |
|---|---|
| Code | `/opt/hermes` → `/opt/hermes-releases/<sha12>/app`: one release per upstream commit, built by **that commit's own `scripts/install.sh`** (upstream's native install: `pm` brings Python 3.14, Node, uv, Chromium, ffmpeg, ripgrep into `app/tools`), run as `hermes` with a throwaway HOME, then sealed root-owned; the previous release kept |
| Python environment | `/opt/data/installs/<key>` (pm keeps it in HERMES_HOME; `key` = sha256 of the app path), removed with its release. `config.yaml` is saved to `data/backups/config.yaml.before-<sha12>` before each build (the installer migrates it) |
| Legacy | releases built before upstream's `pm` (`<sha12>/` with a python3.13 `.venv`, `/opt/hermes-toolchain`) still run and roll back; no longer built |
| Data | `/srv/hermes/data`, also at `/opt/data` (symlink: every `/opt/data/...` path in config, MCP and `.env` still works) |
| Services | `hermes-gateway.service` (generated by the active release at every activate, + our drop-in `hermes-gateway.service.d/10-command-center.conf`: env, limits 10G / 6 CPU / 4096 tasks; alias `hermes-gateway-<hash>.service`, the name Hermes looks for), `hermes-dashboard.service` |
