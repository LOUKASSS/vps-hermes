...
- Pinned **yazio-mcp 0.0.14** (fliptheweb/yazio-mcp), installed once with npm at
  `/opt/data/profiles/health/mcp/node/` (host: `/srv/hermes/data/profiles/health/mcp/node/`)
- Server `yazio` = `node …/node_modules/yazio-mcp/dist/index.js`, all 15 tools enabled. Health profile, 2026-09-19.
...
```bash
hermes -p health mcp test yazio
```

If auth fails: `hermes -p health secrets bitwarden sync` must list `YAZIO_USERNAME` / `YAZIO_PASSWORD`, then reload MCP. Do not recreate a plaintext env file.
