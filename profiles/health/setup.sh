#!/usr/bin/env bash
# health profile: build the MCP servers config.yaml declares (mcp_servers: hevy, yazio, renpho)
# from what mcp/ in this directory vendors. Runs inside hermes-agent as the runtime user after
# `profiles.sh install health` (PROFILE_DIR=/opt/data/profiles/health, DIST_DIR=/opt/data/distributions/health).
#
#   mcp/node/               hevy-mcp + yazio-mcp from the npm registry, exact tree pinned by package-lock.json
#   mcp/renpho-mcp-server/  renpho-mcp-server 1.2.0 source (StartupBros/renpho-mcp-server@69eb7aa, MIT),
#                           dependencies pinned by package-lock.json, compiled here with tsc
#
...
set -euo pipefail
src="${DIST_DIR:?}/mcp"; mcp="${PROFILE_DIR:?}/mcp"
